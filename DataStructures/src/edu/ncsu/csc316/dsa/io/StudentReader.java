...
public class StudentReader {
	...
	private static Student processLine(String line) {
		...
		String sGPA = scanner.next();
		double gPA = Double.parseDouble(sGPA);
		String creditHS = scanner.next();
		int creditHours = Integer.parseInt(creditHS);
		scanner.close();
		Student c = new Student(null, null, 0, 0, 0, null);
		c.setFirst(first);
		c.setLast(last);
		c.setGpa(gPA);
		c.setId(id);
		c.setUnityID(unityID);
